      ...
      {screen === 'start' ? (
        ...
        <aside className="game-panel" aria-live="polite">
          <div className="game-heading">
            <p className="eyebrow">
              {isSeeded
                ? screen === 'finished' ? 'Shared map complete' : `Shared map · ${challenge.id}`
                : screen === 'finished' ? 'Daily complete' : formattedDate}
            </p>
            <h1>
              {screen === 'finished'
                ? isSeeded ? 'Your Hamburg' : 'Today’s Hamburg'
                : 'Name the Pins'}
            </h1>
            <p>
              {screen === 'finished'
                ...
                : 'A Guess checks every unsolved Pin.'}
            </p>
          </div>
          ...
          {screen === 'game' ? (
            <>
              ...
              {feedback ? (
                ...
                <p className="input-hint">Choose from all 104 official Stadtteile.</p>
              )}

              <div className="budget-block">
                <span>Guess Budget</span>
                <strong>{progress?.budgetRemaining ?? 10}</strong>
                <div className="budget-track" aria-hidden="true">
                  {Array.from({ length: challenge?.initial_budget ?? 10 }, (_, index) => (
                    <i key={index} className={index < (progress?.budgetRemaining ?? 10) ? 'budget-unit budget-unit--left' : 'budget-unit'} />
                  ))}
                </div>
              </div>
              <button
          ...
          <footer className="map-key">
            <LocateFixed size={16} />
            <span>{screen === 'finished' ? 'Green solved · amber revealed · orange/red missed' : 'Green solved · orange/red missed'}</span>
          </footer>
        </aside>
      )}
